<?php
/**
 * Zaphpa PHP Router modified for Soarin
 *
 * @version 1.0
 *
 */
...
class Zaphpa_Callback_Util {
	...
	public static function getCallback($callback, $file = null) {
		...
		if (is_array($callback)) {
			...
			// Class not found exception handler (added by SoarinPHP)
			try {
...
class Zaphpa_Template {
	...
	public function addQueryParam($name, $pattern = '', $defaultValue = null) {
		...
		$this -> params[$name] = (object) array('pattern' => sprintf($pattern, $name), 'value' => $defaultValue);
	}
	...
	public static function addGlobalQueryParam($name, $pattern = '', $defaultValue = null) {
		...
		self::$globalQueryParams[$name] = (object) array('pattern' => sprintf($pattern, $name), 'value' => $defaultValue);
	}
...
class Zaphpa_Request {
	...
	function __construct() {
		...
		switch ($this->method) {
			...
			default :
				...
				// people do use query params with POST, PUT, and DELETE
	...
	public function common_aliases() {
		return array('html' => 'text/html', 'txt' => 'text/plain', 'css' => 'text/css', 'js' => 'application/x-javascript', 'xml' => 'application/xml', 'rss' => 'application/rss+xml', 'atom' => 'application/atom+xml', 'json' => 'application/json', 'jsonp' => 'text/javascript', );
	}
